/**
 * Skript för att fixa problemet med dubletter på Utridarevägen 3B
 * Skriptet skapar kopior av dessa lägenheter så att varje hyresgäst får sin egen
 */
...
async function main() {
  try {
    console.log('Startar fix för Utridarevägen 3B...');
    
    // Upprätta databasanslutning
    const connection = await mysql.createConnection(dbConfig);
    console.log('Databasanslutning upprättad');
    
    // Hitta lägenheter på Utridarevägen 3B med flera hyresgäster
    const [duplicateApartments] = await connection.execute(`
      ...
      WHERE 
        a.street LIKE 'Utridarevägen%' AND a.number = '3B'
      GROUP BY 
        a.id
      HAVING 
    ...
    console.log(`Hittade ${duplicateApartments.length} lägenheter på Utridarevägen 3B med flera hyresgäster`);
    
    // Om inga duplicerade lägenheter hittades, kontrollera alla på Utridarevägen
    if (duplicateApartments.length === 0) {
      console.log('Söker efter alla lägenheter på Utridarevägen med flera hyresgäster...');
      
      const [allDuplicates] = await connection.execute(`
        SELECT 
          a.id AS apartment_id, 
          a.street, 
          a.number, 
      ...
      console.log(`Hittade ${allDuplicates.length} lägenheter på Utridarevägen med flera hyresgäster`);
      
      if (allDuplicates.length > 0) {
        // Använd dessa istället
        duplicateApartments.push(...allDuplicates);
      }
    }
    
    // Om fortfarande inga duplicerade lägenheter, sök efter alla duplicerade lägenheter oavsett adress
    if (duplicateApartments.length === 0) {
      console.log('Söker efter alla lägenheter med flera hyresgäster...');
      
      const [anyDuplicates] = await connection.execute(`
        SELECT 
          a.id AS apartment_id, 
          a.street, 
          a.number, 
      ...
      console.log(`Hittade ${anyDuplicates.length} lägenheter med flera hyresgäster`);
      
      if (anyDuplicates.length > 0) {
        // Använd dessa istället
        duplicateApartments.push(...anyDuplicates);
      }
    }
